    ...
    class Backgroundtask extends AsyncTask<Integer,Integer,Integer>{
        ...
        protected Integer doInBackground(Integer... integers) { //가변매개변수  integers 는 integer[]
            ...
            for(backValue1 = 0 ; backValue1 < integers[0]; backValue1++){
                if(backValue1 % 10 == 0 ){
                    publishProgress(backValue1);    // progress 상태를 update 뽑아냄 --> onprogressUpdate 호출되고 매개변수
                }
            ...
            return backValue1;  //onpostExecute의 integer로 넘어감
        ...
        //백스라운드 작업 도중 (여러번) 호출 가능, 진행상황 업데이트, 중가중간 UI 업데이트시 사용
        @Override
        protected void onProgressUpdate(Integer... values) {
            Log.d("myapp", "progress" + values[0] + "%");   //publishprogress(i)가 보내주 값
            super.onProgressUpdate(values);

            tvBackValue1.setText("onprogressUpate: " + values[0]);  // handler 없어도 메인 UI 접근 가능
        ...
        @Override
        protected void onPostExecute(Integer integer) { //doinbackground에서 리턴한 값을 매개변수로 삼는다
            Log.d("myapp","Result : " + integer);
            super.onPostExecute(integer);

            tvBackValue1.setText("onPostExecute" + integer);
        }
