...
public class Main3Activity extends AppCompatActivity {
    ...
    Handler mHandler = new Handler(){
        ...
        public void handleMessage(@NonNull Message msg) {
            doUpload(msg.what); //메세지값을 업로드
        }
    };

    void doUpload(int n){
        Toast.makeText(getApplicationContext(), n + ": 업로드를 완료함",Toast.LENGTH_LONG).show();
    }
    ...
    public  void mOnClick3(View v){
        new AlertDialog.Builder(this)
                ...
                .setPositiveButton("예", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                       // View (Button) 통해서도 Runnable 을 생성해서 보낼수있다
                        Button btnup = findViewById(R.id.btnUpload3);
